...
```
SELECT 
    o.id_oficina AS office_id,
    c.nombre_ciudad AS city_name,
    o.numero_telefono_oficina AS office_phone_number
FROM 
    oficina AS o
INNER JOIN
    ciudad AS c ON c.id_ciudad = o.id_ciudad
INNER JOIN
    region AS r ON r.id_region = c.id_region
INNER JOIN
    pais AS p ON p.id_pais = r.id_pais
WHERE 
...
```
SELECT 
    e.nombre_empleado AS employee_first_name,
    e.apellido_empleado AS employee_first_surname,
    e.segundo_apellido_empleado AS employee_last_surname
FROM
    empleado AS e
WHERE
    e.id_jefe = 7;
